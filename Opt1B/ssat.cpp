...
//Splitting Heuristic 1
//Look for shortest (non unit) clause
void shortClauses(std::vector<double>*,std::vector<std::vector<int>>*, std::vector<int>*, std::vector<int>*, std::vector<std::vector<int>>*);
...
//Splitting Heuristic 1
//Look for shortest (non unit) clause
void shortClauses(std::vector<double>* variables,std::vector<std::vector<int>>* clauses, std::vector<int>* sats, std::vector<int>* assignments, std::vector<std::vector<int>>* varsByClause)

{
    //Store size of clause
    int clauseSize = 0;

    //Store clause number of largest clause
    int clauseNumber = 0;
  
    //Loops through all of the clauses (why signed int)
    for (int c = 0; c < (signed int)clauses->size(); c++) {
	...
	//Longest clause so far
	if (clauses->at(c).size() > clauseSize){
	    clauseSize = clauses->at(c).size();
	    clauseNumber = c;
      
	}
    }

    //Make sure first variable is in the current block
  
    //Get first variable of shortest clause
    int firstVar = clauses->at(clauseNumber).begin();

    //Determine if variable is a choice or chance variable
    double varAss = variables->at(abs(firstVar) -1);
  
  

    //Iterate through assignments until we arrive at an unassigned variable
    for (int v = 0; v < (signed int)assignments->size(); v++) {

	//Variable is unassigned (could be a chance variable too)
	if (assignments->at(v) == 0){

	    //Variable is not in the same type of block as its own type
	    if(variables->at(v) != varAss) {

		//Find some way of trying the next variable or next shortest clause
		break;
	    }

	    //Iterate through rest of block
	    for (int b = v; b < (signed int)assignments->size(); c++) {

		//Reached end of block
		if (variables->at(b) != varAss) {

		    //Find some way of trying the next variable or next shortest clause
		    break;
		}

		//Variable is found within the block
		if (variables->at(b) == firstVar) {

		    //Assign variable in assignments vector
		    assignments->at(firstVar) = firstVar / abs(firstVar);

		    //Check if all clauses are satisfied
		    satisfyClauses(clauseNumber, &pureCopy, &pureSats, &assignments, &pureVBC);
	...
	//Iterate through the block until the end of it
	bool block = choice;

	//Iterate through rest of block
	for (int b = v; b < (signed int)clauses->size(); c++) {

	    //Variable is choice variable
	    if (variables->at(b) == -1) {

		block = true;

		//Variable is a chance variable
	    } else {
	  
		block = false;
	    }

	    //Block ended without variable being found
	    //Need some way to get the next shortest clause by either going to the start of this function
	    //and picking the second longest clause or something like that (recursion?)
	...
	MOMS maximal occurenes of minimal size
